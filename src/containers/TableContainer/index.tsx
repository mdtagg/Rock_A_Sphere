...
const TableContainer = () => {
  ...
  const [rockTypes, setRockTypes] = useState<Map<any, any> | undefined>(
    undefined,
  );
  const [rockData, setRockData] = useState<RockDataType | undefined>(undefined);

  const tableDataStyle =
    "flex flex-col border-r-2 border-b-2 border-black items-center justify-end w-1/5 p-1 h-full sm:p-0";
  ...
  const hide = rainData.buttonTitle !== "Wet Rock" ? "hidden" : "flex";
  ...
  useEffect(() => {
    (async function () {
      ...
      rockTypes.success.data.forEach((val: TRockType) =>
        rockTypeMap.set(val.lith_id, val),
      );
  ...
  return (
    <table
      className={`${hide} flex-col w-2/5 bg-slate-200/50 border-t-2 border-l-2 border-black ml-11 sm:w-full sm:m-0 wide:w-screen wide:m-0 animate-fadeIn`}
    >
      <thead>
        ...
        <tbody>
          <tr className="flex items-center h-32 sm:h-[100px] wide:h-[100px] wide:text-sm">
            <td className={tableDataStyle}>
              ...
              <a className={tableAnchorStyle} href="https://open-meteo.com/">
            ...
            <td className={tableDataStyle}>
              <RockList list={rockData!.primaryRockType} />
              <a
            ...
            <td className={tableDataStyle}>
              <DaysToClimb rockData={rockData} weatherData={weatherData} />
            </td>
          </tr>
        </tbody>
      )}
